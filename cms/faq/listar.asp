...
If Palavra <> "" then
	Busca = "WHERE faq_nome LIKE '%" & Palavra & "%' OR faq_desc LIKE '%" & Palavra & "%' "
End If

Select case Ordem
	Case 0
		Ordenar = "ORDER BY faq_post"
	Case 1
		Ordenar = "ORDER BY faq_post DESC"
	Case 2
		Ordenar = "ORDER BY faq_nome"
	Case 3
		Ordenar = "ORDER BY faq_nome DESC"
End Select

Set RS = Server.CreateObject("Adodb.recordset")
	SQL = "SELECT * FROM faq " & Busca & Ordenar
	RS.cursorlocation = 3
	RS.Open SQL,Conn
...
<script>
	helpTXT =	'Gerencia a sessão "dúvidas" listada no portal. É possível adicionar, excluir ou editar estas informações. ' +
...
If fso.fileExists(Server.MapPath("../../imagens/faq/" & cStr(RS("faq_id")) & ".jpg")) = True Then
	thumbImg = "../../imagens/faq/" & cStr(RS("faq_id")) & ".jpg"
...
<table border="0" cellspacing="0" cellpadding="0" id="tableList">
  <tr>
    ...
    <td id="divisor"></td>
    <td id="desc">
    	<a class="infoTitle"><%= RS("faq_nome")%></a>
        <a class="infoSub"><strong>Data de postagem:</strong> <% =RS("faq_post") %></a>
    </td>
    <td id="fim"></td>
    <td id="icon" width="40"><a href="javascript:subEditar(<%= contador %>,<%= RS("faq_id") %>)"><img src="../interface/icons_edit.jpg" title="Editar"></a><a href="../includes/msgPop.asp?Id=<% =RS("faq_id") %>" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_del.jpg" title="Deletar"></a></td>
  </tr>
</table>
