...
If Palavra <> "" then
	Busca = "WHERE agenda_nome LIKE '%" & Palavra & "%' OR agenda_desc LIKE '%" & Palavra & "%' "
End If

Select case Ordem
	Case 0
		Ordenar = "ORDER BY agenda_post"
	Case 1
		Ordenar = "ORDER BY agenda_post DESC"
	Case 2
		Ordenar = "ORDER BY agenda_nome"
	Case 3
		Ordenar = "ORDER BY agenda_nome DESC"
End Select

Set RS = Server.CreateObject("Adodb.recordset")
	SQL = "SELECT * FROM agenda " & Busca & Ordenar
	RS.cursorlocation = 3
	RS.Open SQL,Conn
...
<script>
	helpTXT =	'Gerencia a sessão "agenda" listada no portal. É possível adicionar, excluir ou editar estas informações. ' +
...
If fso.fileExists(Server.MapPath("../../imagens/agenda/" & cStr(RS("agenda_id")) & ".jpg")) = True Then
	thumbImg = "../../imagens/agenda/" & cStr(RS("agenda_id")) & ".jpg"
...
<table border="0" cellspacing="0" cellpadding="0" id="tableList">
  <tr>
    ...
    <td id="divisor"></td>
    <td id="desc">
    	<a class="infoTitle"><strong><%= FormatDateTime(RS("agenda_data"), 2) %></strong> - <%= RS("agenda_nome") %></a>
        <a class="infoSub"><strong>Data de postagem:</strong> <% =RS("agenda_post") %></a>
    </td>
    <td id="fim"></td>
    <td id="icon" width="80"><a href="../includes/msgPop.asp?Id=<% =RS("agenda_id") %>&Engine=destaque" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_destaque_<%= RS("agenda_destaque") %>.jpg" title="Habilitar/Desabilitar destaque"></a><a href="javascript:subEditar(<%= contador %>,<%= RS("agenda_id") %>)"><img src="../interface/icons_edit.jpg" title="Editar"></a><a><img src="../interface/icons_no.jpg"></a><a href="../includes/msgPop.asp?Id=<% =RS("agenda_id") %>" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_del.jpg" title="Deletar"></a></td>
  </tr>
</table>
