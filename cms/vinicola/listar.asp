...
If Palavra <> "" then
	Busca = "WHERE vinicola_nome LIKE '%" & Palavra & "%' OR vinicola_desc LIKE '%" & Palavra & "%' "
End If

Select case Ordem
	Case 0
		Ordenar = "ORDER BY vinicola_post"
	Case 1
		Ordenar = "ORDER BY vinicola_post DESC"
	Case 2
		Ordenar = "ORDER BY vinicola_nome"
	Case 3
		Ordenar = "ORDER BY vinicola_nome DESC"
End Select

Set RS = Server.CreateObject("Adodb.recordset")
	SQL = "SELECT * FROM vinicola " & Busca & Ordenar
	RS.cursorlocation = 3
	RS.Open SQL,Conn
...
<script>
	helpTXT =	'Gerencia a sessão "Vinícola" listada no portal. É possível adicionar, excluir ou editar estas informações. ' +
...
If fso.fileExists(Server.MapPath("../../imagens/vinicola/" & cStr(RS("vinicola_id")) & ".jpg")) = True Then
	thumbImg = "../../imagens/vinicola/" & cStr(RS("vinicola_id")) & ".jpg"
...
<table border="0" cellspacing="0" cellpadding="0" id="tableList">
  <tr>
    ...
    <td id="divisor"></td>
    <td id="desc">
    	<a class="infoTitle"><%= RS("vinicola_nome")%></a>
        <a class="infoSub"><strong>Data de postagem:</strong> <% =RS("vinicola_post") %></a>
    </td>
    <td id="fim"></td>
    <td id="icon" width="80"><a href="../includes/msgPop.asp?Id=<% =RS("vinicola_id") %>&Engine=destaque" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_destaque_<%= RS("vinicola_destaque") %>.jpg" title="Habilitar/Desabilitar destaque"></a><a href="javascript:subEditar(<%= contador %>,<%= RS("vinicola_id") %>)"><img src="../interface/icons_edit.jpg" title="Editar"></a><a><img src="../interface/icons_no.jpg"></a><a href="../includes/msgPop.asp?Id=<% =RS("vinicola_id") %>" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_del.jpg" title="Deletar"></a></td>
  </tr>
</table>
