...
If Palavra <> "" then
	Busca = "WHERE abertura_nome LIKE '%" & Palavra & "%' OR abertura_desc LIKE '%" & Palavra & "%' "
End If

Select case Ordem
	Case 0
		Ordenar = "ORDER BY abertura_ordem"
	Case 1
		Ordenar = "ORDER BY abertura_ordem"
	Case 2
		Ordenar = "ORDER BY abertura_nome"
	Case 3
		Ordenar = "ORDER BY abertura_nome DESC"
End Select

Set RS = Server.CreateObject("Adodb.recordset")
	SQL = "SELECT * FROM abertura " & Busca & Ordenar
	RS.cursorlocation = 3
	RS.Open SQL,Conn
...
<script>
	helpTXT =	'Gerencia as imagens exibidas na página inicial do portal. É possível adicionar, excluir ou editar estas informações. ' +
				...
				'A ordem dos itens pode ser alterada através deste abertura adminstrativo.';
</script>
</head>
<body>
...
If fso.fileExists(Server.MapPath("../../imagens/abertura/" & cStr(RS("abertura_id")) & ".jpg")) = True Then
	thumbImg = "../../imagens/abertura/" & cStr(RS("abertura_id")) & ".jpg"
...
<table border="0" cellspacing="0" cellpadding="0" id="tableList">
  <tr>
    ...
    <td id="desc">
    	<a class="infoTitle"><%
			If RS("abertura_nome") <> "" Then
				Response.Write RS("abertura_nome")
			Else
				Response.Write "Sem nome"
			End if
		%></a>
        <a class="infoSub"><strong>Ordem de exibição:</strong> <% =RS("abertura_ordem") %>º</a>
    </td>
    <td id="fim"></td>
    <td id="icon" width="80"><a href="ordem.asp?Id=<% =RS("abertura_id") %>&Ordem=<% =RS("abertura_ordem") %>" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_ordem.jpg" title="Alterar ordem"></a><a href="javascript:subEditar(<%= contador %>,<%= RS("abertura_id") %>)"><img src="../interface/icons_edit.jpg" title="Editar"></a><a><img src="../interface/icons_no.jpg"></a><a href="../includes/msgPop.asp?Id=<% =RS("abertura_id") %>" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_del.jpg" title="Deletar"></a></td>
  </tr>
</table>
