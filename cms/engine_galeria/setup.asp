...
Set fso = CreateObject("Scripting.FileSystemObject")
	Set folder = fso.GetFolder(Server.Mappath("../../imagens/engine_galeria/" & Id & "/thumbs/")).Files
...
function thumbShow() {
	...
	for (var i = 0; i < thumbCheck.length; i++) {
		...
		"url(" + "../../imagens/engine_galeria/<%= Id  %>/thumbs/<%= Id  %>_"+ (i+1) +".jpg?" +
		(new Date()).getTime() + ")";
	}
...
function zoomPop(quem) {
	...
	var zoomImg = "../../imagens/engine_galeria/<%= Id  %>/fotos/<%= Id  %>_"+ zoomCheck.value +".jpg";
