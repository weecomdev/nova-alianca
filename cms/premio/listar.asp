...
If Palavra <> "" then
	Busca = "WHERE premio_nome LIKE '%" & Palavra & "%' OR premio_desc LIKE '%" & Palavra & "%' "
End If

Select case Ordem
	Case 0
		Ordenar = "ORDER BY premio_post"
	Case 1
		Ordenar = "ORDER BY premio_post DESC"
	Case 2
		Ordenar = "ORDER BY premio_nome"
	Case 3
		Ordenar = "ORDER BY premio_nome DESC"
End Select

Set RS = Server.CreateObject("Adodb.recordset")
	SQL = "SELECT * FROM premio " & Busca & Ordenar
	RS.cursorlocation = 3
	RS.Open SQL,Conn
...
<script>
	helpTXT =	'Gerencia a sessão "Premiações" listada no portal. É possível adicionar, excluir ou editar estas informações. ' +
...
If fso.fileExists(Server.MapPath("../../imagens/premio/" & cStr(RS("premio_id")) & ".jpg")) = True Then
	thumbImg = "../../imagens/premio/" & cStr(RS("premio_id")) & ".jpg"
...
<table border="0" cellspacing="0" cellpadding="0" id="tableList">
  <tr>
    ...
    <td id="divisor"></td>
    <td id="desc">
    	<a class="infoTitle"><strong><%= FormatDateTime(RS("premio_data"), 2) %></strong> - <%= RS("premio_nome") %></a>
        <a class="infoSub"><strong>Data de postagem:</strong> <% =RS("premio_post") %></a>
    </td>
    <td id="fim"></td>
    <td id="icon" width="80"><a href="../includes/msgPop.asp?Id=<% =RS("premio_id") %>&Engine=destaque" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_destaque_<%= RS("premio_destaque") %>.jpg" title="Habilitar/Desabilitar destaque"></a><a href="javascript:subEditar(<%= contador %>,<%= RS("premio_id") %>)"><img src="../interface/icons_edit.jpg" title="Editar"></a><a><img src="../interface/icons_no.jpg"></a><a href="../includes/msgPop.asp?Id=<% =RS("premio_id") %>" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_del.jpg" title="Deletar"></a></td>
  </tr>
</table>
