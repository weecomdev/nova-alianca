...
Select case Ordem
	Case 0
		Ordenar = "ORDER BY galeria_post DESC"
	Case 1
		Ordenar = "ORDER BY galeria_post DESC"
	Case 2
		Ordenar = "ORDER BY galeria_nome"
	Case 3
		Ordenar = "ORDER BY galeria_nome DESC"
End Select

Set RS = Server.CreateObject("Adodb.recordset")
	SQL = "SELECT * FROM galeria " & Busca & Ordenar & ";"
	RS.cursorlocation = 3
...
<table border="0" cellspacing="0" cellpadding="0" id="tableLine">
  <tr>
    <td id="inicio"></td>
    <td id="desc"><a class="infoTitle"><%= RS("galeria_nome")%></a></td>
    <td id="fim"></td>
    <td id="icon" width="120"><a href="javascript:subEditar(<%= contador %>,<%= cStr(RS("galeria_id")) %>,'setup')"><img src="../interface/icons_setup.jpg" title="Adicionar/Excluir fotos"></a><a href="javascript:subEditar(<%= contador %>,<%= cStr(RS("galeria_id")) %>)"><img src="../interface/icons_edit.jpg" title="Editar"></a><a href="../includes/msgPop.asp?Id=<%= cStr(RS("galeria_id")) %>" target="msgPop" onClick="showPop(event)"><img src="../interface/icons_del.jpg" title="Deletar"></a></td>
  </tr>
</table>
