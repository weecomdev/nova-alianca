...
class OpcoesDicionarioDao {
	...
	private function prepareStatement(OpcaoDicionario $opcoesDicionario) {
        ...
        if (!is_null($opcoesDicionario->CampoTabela)){
				...
				$values[$v++] = preg_replace('#[^\pL\pN./\' -]+#' , '', $this->db->qstr($opcoesDicionario->CampoTabela));
        }
        
        if (!is_null($opcoesDicionario->Opcao)){
                $columns[$c++] = "Opcao";
				$values[$v++] = preg_replace('#[^\pL\pN./\' -]+#' , '', $this->db->qstr($opcoesDicionario->Opcao));
        }

        if (!is_null($opcoesDicionario->TamanhoDescricao)){
                $columns[$c++] = "TamanhoDescricao";
				$values[$v++] = preg_replace('#[^\pL\pN./\' -]+#' , '', $this->db->qstr($opcoesDicionario->TamanhoDescricao));
        }

        if (!is_null($opcoesDicionario->Descricao60)){
                $columns[$c++] = "Descricao60";
				$values[$v++] = str_replace(array('{','}','[',']','<','>','=','"'),'',$this->db->qstr($opcoesDicionario->Descricao60));
        }
