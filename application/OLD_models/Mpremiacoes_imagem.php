...
class MPremiacoes_imagem extends CI_Model {

    function __construct(){ parent::__construct(); }

   
  /* save data */
  var $tbl_main = 'tbl_premiacoes_images';
  var $id_main = 'premiacao_image_id';
  ...
  public function getNextOrder($id) {
    ...
    public function countImages($premiacao_id)
    {
        $this->db->select('count(*) as total');
        $this->db->where('premiacao_id',$premiacao_id);
      $query = $this->db->get($this->tbl_main);
        $row = $query->row();
    return $row->total;
    }
    ...
    private function deleteImage($id){
      if(!empty($id)){
        $item = $this->get($id);
      if(!empty($item->image)){
        $img = PREMIACOES_IMAGES_UPLOAD_PATH.$item->image;
        @unlink($img);
        $img = PREMIACOES_IMAGES_UPLOAD_PATH.$item->thumb_image;
        @unlink($img);
      }
      return TRUE;
      }
