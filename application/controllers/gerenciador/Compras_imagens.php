...
class Compras_imagens extends MY_Controller {
    ...
    {
        $this->data['id'] = $id;
        $this->data['items'] = $this->MCompras_imagem->getAll($id);
        $this->load->view('gerenciador/compras/images/index', $this->data);
    }

    public function reorder()
    {
        $items = $this->input->post('data');
        if ($this->MCompras_imagem->reorder($items, true)) $this->output->set_output('1');
        else{
    ...
    {
        $this->data['id'] = $id;
        $this->load->view('gerenciador/compras/images/add', $this->data);
    }
    ...
    function upload($id=0)
    {
        ...
        {
            $base_dir = COMPRAS_IMAGES_UPLOAD_PATH;
            ...
            else
            {
                ...
                $this->MCompras_imagem->save($data,$id);
    ...
    public function delete($gallery_image_id,$id)
    {
        if (!empty($gallery_image_id)) {
            if ($this->MCompras_imagem->delete($gallery_image_id,$id)) {
                $this->setMessage("Imagem excluída com sucesso!");
                redirect('gerenciador/compras_imagens/index/'.$id);
            }
        }
        $this->setMessage("Item não encontrado.", true);
        redirect('gerenciador/compras_imagens');
    }
