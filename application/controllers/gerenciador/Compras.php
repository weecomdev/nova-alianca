...
class Compras extends MY_Controller {
	...
	{
		$this->data['items'] = $this->MCompras->getAll();
		$this->load->view('gerenciador/compras/index', $this->data);
	}
	
	public function reorder($id=0)
	{
		$items = $this->input->post('data');
		if ($this->MCompras->reorder($items)) $this->output->set_output('1');
		else{ 
	...
	public function add()
	{
		$this->load->view('gerenciador/compras/form', $this->data);
	}
	...
	public function edit($id=0)
	{
		if(!empty($id))
		{
			$this->data['item'] = $this->MCompras->get($id);
			$this->load->view('gerenciador/compras/form', $this->data);
		}
		else
		{
	...
	public function save(){
		...
		else
		{
			...
			// if($upload_data)
			// {
				if($this->MCompras->save())
				{
					$this->setMessage($this->lang->line('message_item_saved'));
					redirect('gerenciador/compras');
				}
				else
				{
