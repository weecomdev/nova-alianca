...
class Users extends MY_Controller {
	...
	{
		$this->data['items'] = $this->User->getAll();
		$this->load->view('gerenciador/users/index', $this->data);
	}
	
	public function add()
	{
		$this->load->view('gerenciador/users/form', $this->data);
	}
			...
			else
			{
				if ($this->input->post('password') != $this->input->post('confirm_password')){
					...
					if($this->User->save())
					{
						...
						if($this->session->userdata('user_level') == 1) redirect('gerenciador/users');
