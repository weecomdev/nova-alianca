...
class MNoticia_imagem extends CI_Model {

    function __construct(){ parent::__construct(); }

   
  /* save data */
  var $tbl_main = 'tbl_noticias_images';
  var $id_main = 'noticia_image_id';
  ...
  public function getNextOrder($id) {
    ...
    public function countImages($noticia_id)
    {
        $this->db->select('count(*) as total');
    ...
    private function deleteImage($id){
      if(!empty($id)){
        $item = $this->get($id);
      if(!empty($item->image)){
        $img = NOTICIAS_IMAGES_UPLOAD_PATH.$item->image;
        @unlink($img);
        $img = NOTICIAS_IMAGES_UPLOAD_PATH.$item->thumb_image;
        @unlink($img);
      }
      return TRUE;
      }
