...
class Noticias_imagens extends MY_Controller {
    ...
    {
        $this->data['id'] = $id;
        $this->data['items'] = $this->MNoticia_imagem->getAll($id);
        $this->load->view('gerenciador/noticias/images/index', $this->data);
    }

    public function reorder()
    {
        $items = $this->input->post('data');
        if ($this->MNoticia_imagem->reorder($items, true)) $this->output->set_output('1');
        else{
    ...
    {
        $this->data['id'] = $id;
        $this->load->view('gerenciador/noticias/images/add', $this->data);
    }
    ...
    function upload($id=0)
    {
        ...
        {
            $base_dir = NOTICIAS_IMAGES_UPLOAD_PATH;
            ...
            else
            {
                ...
                $this->MNoticia_imagem->save($data,$id);
    ...
    public function delete($gallery_image_id,$id)
    {
        if (!empty($gallery_image_id)) {
            if ($this->MNoticia_imagem->delete($gallery_image_id,$id)) {
                $this->setMessage("Imagem excluída com sucesso!");
                redirect('gerenciador/noticias_imagens/index/'.$id);
            }
        }
        $this->setMessage("Item não encontrado.", true);
        redirect('gerenciador/noticias_imagens');
    }
