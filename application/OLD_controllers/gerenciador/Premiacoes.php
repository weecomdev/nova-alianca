...
class premiacoes extends MY_Controller {
	...
	{
		$this->data['items'] = $this->MPremiacoes->getAll();
		$this->load->view('gerenciador/premiacoes/index', $this->data);
	}
	
	public function reorder($id=0)
	{
		$items = $this->input->post('data');
		if ($this->MPremiacoes->reorder($items)) $this->output->set_output('1');
		else{ 
	...
	public function add()
	{
		$this->load->view('gerenciador/premiacoes/form', $this->data);
	}
	...
	public function edit($id=0)
	{
		if(!empty($id))
		{
			$this->data['item'] = $this->MPremiacoes->get($id);
			$this->load->view('gerenciador/premiacoes/form', $this->data);
		}
		else
		{
	...
	public function save(){
		...
		else
		{
			...
			// if($upload_data)
			// {
				if($this->MPremiacoes->save())
				{
					$this->setMessage($this->lang->line('message_item_saved'));
					redirect('gerenciador/premiacoes');
				}
				else
				{
