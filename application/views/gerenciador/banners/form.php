...
<?php echo form_open_multipart('gerenciador/banners/save', array('class'=>'form-horizontal', 'role'=>'form'));?>
	<fieldset>
        ...
        <div class="form-group">
            <label class="col-sm-2 control-label" for="fileInput">Imagem Principal ES (opcional):</label>
            <div class="col-xs-5">
                <input id="fileInput" name="img_es" class="input-file" type="file" />
                <p class="help-block">(*.png *.jpg *.gif 700x700)
                    <?php if (!empty($item->img_es)){ ?>
                        (<a href="<?php echo site_url(BANNERS_PATH.$item->img_es); ?>" title="ver imagem atual" target="_blank">Ver Imagem Atual</a>)
                    <?php }?>
                </p>
            </div>
        </div>
